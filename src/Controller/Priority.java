...
public class Priority extends ProcessController {
    ...
    private HashMap<String, Integer> processPriority = new HashMap<>();
    ...
    public void execute() {
        ...
        for (Process process : logic) {
            ...
            processPriority.put(process.getName(), process.getPriority());
            processAge.put(process.getName(),0);
        }
        ...
        for (Process process : sortedProcesses) {
            try {
                System.out.println(process.getName() + "               " +
                        (endTime.get(process.getName()) - process.getArrivalTime() - process.getBurstTime()) +
                        "                " + (endTime.get(process.getName()) - process.getArrivalTime()));
    ...
    private Process getHighestPriorityProcess(Vector<Process> processes, int currentTime) {
        ...
        for (Process process : processes) {
            ...
            if (process.getArrivalTime() <= currentTime) {
                if (processPriority.get(process.getName()) < highestPriorityValue) {
                    if (processBurstTime.get(process.getName()) != 0) {
                        highestPriority = process;
                        highestPriorityValue = processPriority.get(process.getName());
                    }
                }
            }
        }
